...
impl TokenAmount {
    ...
    /// This only gets used to calculate the protocol fees and if it is not zero then we always
    /// round up for protocol fees to have minimum 1 token if the protocol fees is not zero
...
mod tests {
    ...
    fn should_return_fee_amount_when_single_input_token_provided() {
        ...
        let fee = token_amount
            .get_percent(Decimal::from_str("0.01").unwrap())
            .unwrap()
            .unwrap();

        assert_eq!(fee.get_total(), Uint128::new(3))
    }

    #[test]
    fn should_return_min_fee_amount_when_fee_and_input_low() {
        let token_amount = TokenAmount::Single(Uint128::new(260));
        let fee = token_amount
            .get_percent(Decimal::from_str("0.01").unwrap())
            .unwrap()
            .unwrap();
    ...
    fn should_return_fee_amount_when_multiple_input_token_provided_and_two_token_amount_are_over() {
        ...
        assert_eq!(fee.get_total(), Uint128::new(26))
    }
