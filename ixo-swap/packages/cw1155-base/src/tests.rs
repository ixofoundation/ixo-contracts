...
fn check_transfers() {
    ...
    assert_eq!(
        execute(
            ...
            Cw1155ExecuteMsg::BatchMint {
                to: user2.clone(),
                batch: vec![
                    (token2.clone(), 1u64.into(), uri.clone()),
                    (token3.clone(), 1u64.into(), uri.clone())
                ],
                msg: None
            },
        )
    ...
    let batch_transfer_msg = Cw1155ExecuteMsg::BatchSendFrom {
        ...
        to: user1.clone(),
        batch: vec![
            (token1.clone(), 1u64.into(), uri.clone()),
            (token2.clone(), 1u64.into(), uri.clone()),
            (token3.clone(), 1u64.into(), uri.clone()),
        ],
        msg: None,
    ...
    assert_eq!(
        execute(
            ...
            Cw1155ExecuteMsg::BatchSendFrom {
                from: user1.clone(),
                to: receiver.clone(),
                batch: vec![(token2.clone(), 1u64.into(), uri.clone())],
                msg: Some(dummy_msg.clone()),
            },
        )
        .unwrap(),
...
fn check_queries() {
    ...
    execute(
        ...
        Cw1155ExecuteMsg::BatchMint {
            ...
            batch: tokens
                .iter()
                .map(|token_id| (token_id.clone(), 1u64.into(), uri.clone()))
                .collect::<Vec<_>>(),
            msg: None,
        },
    )
