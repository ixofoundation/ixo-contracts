...
use cw1155_lp::{ApproveAllEvent, Cw1155BatchReceiveMsg, Cw1155ReceiveMsg, TokenId, TransferEvent};
use cw_utils::{Event, Expiration};

use crate::{
    contract::ExecuteEnv,
    helpers::guard_can_approve,
    state::{APPROVES, BALANCES, MINTER, SUPPLIES, TOKENS},
    ContractError,
};
...
pub fn mint(
    ...
    SUPPLIES.update(deps.storage, &token_id, |supply| -> StdResult<_> {
        Ok(supply.unwrap_or_default().checked_add(amount)?)
    })?;
