...
use cw1155_lp::{Cw1155ExecuteMsg, Cw1155QueryMsg};
...
pub fn query(deps: Deps, env: Env, msg: Cw1155QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        Cw1155QueryMsg::BatchBalanceForTokens { token_ids } => {
            to_binary(&query::batch_balance_for_tokens(deps, token_ids)?)
        }
        Cw1155QueryMsg::BatchBalanceForAll {} => to_binary(&query::batch_balance_for_all(deps)?),
